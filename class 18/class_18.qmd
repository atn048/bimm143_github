...
They make their data available as JSON format API: We can read this into R with the `read_json()` function fro the **jsonlite** package:
...
ggplot(subject) +
  ...
  xlab("Age in years")
```
We can read mroe tables from the CMI-PB database
...
To make sense of all this data we need to "join" (aka "merge" or "link") all these tables together. Only then will you know that a given Ab measurement (from the `ab_titer` table) was was collected on a certain date from a certain date (from the `speciman` table) from a certain wP or aP subject (from the `subject` table).
...
> Q11. How many specimens (i.e. entries in abdata) do we have for each isotype?  IgE: 6698, IgG: 7265, IgG1: 11993, IgG2: 12000, IgG3: 12000, IgG4: 12000 
...
> Q. Is there a difference for these repsponses between aP and wP individuals?
